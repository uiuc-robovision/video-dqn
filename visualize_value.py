...
def build_map_gibson(config, model, house, floor):
    ...
    model.eval()
    resolution = 1500
    data_root = f'{config.VISUALIZATION_DATA_ROOT}/{house.name}_{floor}'
    ...
    for ori in range(0, 4):
        new_map = np.zeros(
            (resolution, resolution, len(gibson_info.class_labels)))
        var_map = np.zeros(
            (resolution, resolution, len(gibson_info.class_labels)))
        dataset = HabitatQVisualizationDatasetGibson(data_root,
                                                     panorama=config.PANORAMA
                                                     or config.PREVIOUS_IMAGES,
                                                     orientation=ori)
        ...
        for batch in tqdm(data_generator):
            ...
            if config.DISTRIBUTIONAL:
                ...
                values = model(images.to(
                    config.device)).max(2).values.detach().cpu()
                new_map[row, col] = values
                free_map[row, col] = 1
        maps.append(new_map)
    ...
    for ty in types:
        for direct in [0, 1, 2, 3, 'max']:
            for i, label in enumerate(gibson_info.class_labels):
                locs = gibson_info.relevant_locations(
                    agent_location,
                    house.object_locations_for_habitat_dest[label])
                mark_locations = [
                    util.habitat.to_grid(l, resolution) for l in locs
                ]
                ...
                cropped_final = final_map[row_crop[0]:row_crop[1],
                                          col_crop[0]:col_crop[1], :]
                fig = plt.Figure()
                ax = fig.subplots()
                ax.set_title(f'{label}, {direct}, {ty}')
                pos = ax.imshow(cropped_final,
                                cmap='viridis',
                                vmin=vmin,
                                vmax=vmax)
...
if __name__ == '__main__':
    ...
    config = ExperimentConfig(f"logs/trained_ground_truth_panorama",
                              device='cuda:2')
    build_map(config, 99)
