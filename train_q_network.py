...
GIBSON_DATASETS=['gibson','gibson_medium_40k','gibson_medium','gibson_medium_inverse','gibson_medium_noisy_40k','gibson_medium_noisy', 'gibson_medium_noisy_value', 'gibson_medium_noisy_40k_value','gibson_branch','multi_target','multi_target_longer','explore','branch_updated']
...
def build_model(config):
    ...
    if config.VALUE_LEARNING or config.ONE_ACTION:
        ...
        actions = 3
    model = HabitatDQNMultiAction(actions,5,extra_capacity=(config.ARCHITECTURE == 'extra_capacity'),panorama=(config.PANORAMA or config.PREVIOUS_IMAGES))
...
def visualize_house(config,model,house,floor,sample_number):
    ...
    grid=torchvision.utils.make_grid(torch.tensor(images).permute(0,3,1,2))
    config.writer.add_image(f"value_map_{house.name}{floor}({house.data['split_tiny']})",grid,global_step=sample_number)
...
def run_train(config,resume_from = -1):
    ...
    houses_to_render = [("Allensville",0),
            ("Beechwood",1),
            ("Darden",0),
            ("Darden",1),
            ("Markleeville",1),
    ...
    houses_to_render = list(map(lambda x: (get_house(x[0]),x[1]),houses_to_render))
    ...
    if config.DATASET == 'nobias':
        ...
        dataset = HabitatQRandomizedDataset(datafile,
                                      gamma=config.GAMMA,
                                      panorama=config.PANORAMA,known=config.TRAIN_ON_GROUND_TRUTH)
    elif config.DATASET == 'real':
        datafile = '/scratch/mc48/real_videos/frames/'
        dataset = QLearningRealDataset(datafile,one_action=True,confidence_reward=config.CONFIDENCE_REWARD,value_learning=config.VALUE_LEARNING,inverse_actions=config.USE_INVERSE_ACTIONS,previous_images=config.PREVIOUS_IMAGES)
        ...
        dataset = GibsonDataset(datafile,panorama=config.PANORAMA,class_label=config.CLASS_LABEL,inverse_action=config.USE_INVERSE_ACTIONS,random_actions=config.RANDOM_ACTIONS,value_learning=config.VALUE_LEARNING,reward_dist=reward_dist,one_action=config.ONE_ACTION,prev_images=config.PREVIOUS_IMAGES)
        known_gt_data = GibsonDataset(datafile,panorama=config.PANORAMA,known=True,class_label=config.CLASS_LABEL,inverse_action=config.USE_INVERSE_ACTIONS,random_actions=config.RANDOM_ACTIONS,value_learning=config.VALUE_LEARNING,semantic_rewards=config.SEMANTIC_REWARDS,detector_rewards=config.DETECTION_REWARDS,reward_dist=reward_dist,one_action=config.ONE_ACTION,prev_images=config.PREVIOUS_IMAGES)
        ...
        dataset = ReacherDataset(datafile,gamma=config.GAMMA,tip_pos=False,has_reward = False)
        render_dataset = ReacherDataset(datafile,gamma=config.GAMMA,tip_pos=True,downsample=int(5e4))

    elif config.DATASET == 'reacher_replay':
        datafile = 'reacher_data/replay.npy'
        dataset = ReacherDataset(datafile,gamma=config.GAMMA,tip_pos=False,has_reward = True)
        render_dataset = ReacherDataset(datafile,gamma=config.GAMMA,tip_pos=True,downsample=int(5e4),has_reward=True)

    elif config.DATASET == 'reacher_replay_override':
        datafile = 'reacher_data/replay.npy'
        dataset = ReacherDataset(datafile,gamma=config.GAMMA,tip_pos=False,has_reward = True,override_reward=True)
        render_dataset = ReacherDataset(datafile,gamma=config.GAMMA,tip_pos=True,downsample=int(5e4),has_reward=True,override_reward=True)
    ...
    _, test_data = data.random_split(known_gt_data, [len(known_gt_data) - test_size, test_size])
    ...
    def process_batch(batch, compare_ground_truth=False,batch_number=None):
        ...
        if not compare_ground_truth:
            ...
            after_values.shape
            # -1 x 5
            Q_a = after_values.gather(2, best_actions.unsqueeze(2)).detach().squeeze()
    ...
    while sample_number < config.NUM_STEPS:
        ...
        model.set_train()
        optimizer.zero_grad()
        loss = process_batch(next(iterator), compare_ground_truth=config.TRAIN_ON_GROUND_TRUTH,batch_number=sample_number)
        loss.backward()
        optimizer.step()
        ...
        print( f'\rbatch:{sample_number}/{config.NUM_STEPS} avg_loss: {running_loss}', end="")

        if sample_number % 100 == 0:
            config.writer.add_scalar('avg_q_loss/train', running_loss, sample_number)

        # checkpoint and eval
        if sample_number % config.CHECKPOINT_INTERVAL == 0:
            torch.save({'sample_number': sample_number,
...
if __name__ == '__main__':
    ...
    config = ExperimentConfig(args.config, device='cuda',remove=args.delete,resume=args.resume)
